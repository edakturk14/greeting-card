...
test('saved cards and photos survive restart, remain immutable, and reject invalid requests',async()=>{
 ...
 try{
 await start();assert.equal((await post({...card,to:''})).status,400);assert.equal((await post({...card,from:' '})).status,400);assert.equal((await post(card,'https://other.test')).status,403);assert.equal((await post({...card,message:' '})).status,400);assert.equal((await post({...card,message:'a'.repeat(1201)})).status,400);assert.equal((await post({...card,color:'blue'})).status,400);
 ...
 const legacyId='L'.repeat(32);await writeFile(path.join(dir,legacyId+'.json'),JSON.stringify({id:legacyId,to_name:'',from_name:'',cover:'This one’s for you.',message:'Older card',color:'silver',status:'ready',created_at:new Date().toISOString()}));assert.equal((await fetch(base+'/api/cards/'+legacyId)).status,200);
 ...
 const edit=await(await post({...card,message:'Updated note',to:'Maya',from:'Eda',photo:null})).json();assert.notEqual(edit.id,result.id);const nameless=await(await fetch(base+'/api/cards/'+edit.id)).json();assert.equal(nameless.to,'Maya');assert.equal(nameless.from,'Eda');assert.equal((await(await fetch(base+'/api/cards/'+result.id)).json()).message,card.message);
