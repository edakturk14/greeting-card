...
function validate(data){data={...data,to:data?.to??'',from:data?.from??''};const result={};for(const [key,max]of Object.entries({to:60,cover:100,message:1200,from:60})){if(typeof data?.[key]!=='string'||(!['to','from'].includes(key)&&!data[key].trim())||data[key].trim().length>max)throw new PublicError(`Please enter ${key==='to'?'a recipient':key==='from'?'your name':key==='cover'?'cover text':'a message'} (up to ${max} characters).`);result[key]=data[key].trim();}if(!['silver','pink','gold'].includes(data.color))throw new PublicError('Choose silver, pink or gold.');result.color=data.color;return result;}
...
export default async function handler(req,res){
 ...
 try{
 if(url.pathname==='/api/config'&&req.method==='GET')return json(res,200,{ready:storageReady()&&(!(process.env.VERCEL||process.env.APP_ENV==='production')||Boolean(process.env.APP_SECRET?.length>=32)),analytics:enabled(req.headers.host),campaigns:(process.env.ANALYTICS_CAMPAIGNS||'launch,instagram,tiktok,newsletter,friends').split(',')});
 if(url.pathname==='/api/cleanup'&&req.method==='GET'){if(!authorizedCleanup(req))throw new PublicError('Not authorized.',401);return json(res,200,{removed:await cleanup()});}
 ...
 if(match&&req.method==='GET'){
  ...
  return json(res,200,await getCard(match[1]));
 }
...
`);}
 ...
 const assets={'/':'index.html','/style.css':'style.css','/app.js':'app.js','/scratch.js':'scratch.js','/scene.js':'scene.js','/penguin-studio.png':'penguin-studio.png','/pip-cutout.png':'pip-cutout.png','/analytics.js':'analytics.js','/generic-cover.svg':'generic-cover.svg','/social-preview.png':'social-preview.png','/favicon-32.png':'favicon-32.png','/icon-192.png':'icon-192.png','/apple-touch-icon.png':'apple-touch-icon.png','/favicon.ico':'favicon-32.png'};
