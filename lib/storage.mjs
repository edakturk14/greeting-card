...
export async function getCard(id){
 ...
 return {type:row.card_type||'scratch',to:row.to_name??row.to??'',cover:row.cover||row.greeting,message:row.message,from:row.from_name??row.from??'',color:row.color||'silver',photo:row.photo?`/api/cards/${id}/photo`:null,createdAt:row.created_at||row.createdAt};
}
