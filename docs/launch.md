...
Reviewed 17 September 2026. No paid service was provisioned. The project repository is https://github.com/edakturk14/greeting-card.

## Account steps still needed

1. Create a dedicated **Supabase Free** project. Run `db/schema.sql` in its SQL Editor. If the schema was previously applied, rerun it to migrate the name constraints to allow empty names. Add the project URL and **service-role secret** to `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` in `.env.local`. Never paste secrets into chat or client JavaScript.
